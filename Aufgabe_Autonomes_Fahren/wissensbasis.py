...
def erstelle_wissensbasis():
    ...
    F = AtomareFormel("F")  #Fußgänger vor dem Auto erkannt
    H = AtomareFormel("H")  #Hupe aktiviert
    A = AtomareFormel("A")  #Ampel ist rot
    S = AtomareFormel("S")  #Anhalten
    W = AtomareFormel("W")  #Weiterfahren

    # Definieren Sie Ihre logischen Regeln (Formeln)
    wissensbasis = [
        Implikation(F, H),  # Wenn Fußgänger, dann Hupe
        Implikation(H, S),  # Wenn Hupe, dann Anhalten
        Implikation(A, S)   # Wenn rote Ampel, dann Anhalten
    ]
